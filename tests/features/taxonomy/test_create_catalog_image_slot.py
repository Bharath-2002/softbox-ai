...
async def test_creates_a_slot_owned_by_the_category() -> None:
    ...
    use_case = CreateCatalogImageSlot(uow_factory, FakeClock(_NOW))
    ...
    stored = await uow_factory.catalog_image_slots.get(tenant_id, slot.id)
    assert stored is not None and stored.key == "closeup"


async def test_creating_on_an_unknown_category_is_not_found() -> None:
    use_case = CreateCatalogImageSlot(FakeUnitOfWorkFactory(), FakeClock(_NOW))
