...
``key`` format is validated by ``entities.spec_key.validate_key`` — see that
module's docstring for why (it is used as a JSONB key, a Pydantic field
name, and a D14 prompt placeholder segment, so a bad key is far cheaper to
catch here than in any of those three places later).
"""
...
class AttributeDefinition:
    ...
    def create(
        ...
        *,
        ...
        validate_key(key, what="Attribute key")
